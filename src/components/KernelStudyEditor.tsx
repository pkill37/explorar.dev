...
import {
  generateKernelMarkers,
  generateKernelAnnotations,
  MARKER_COLORS,
  MARKER_ICONS,
  type KernelMarker,
  type KernelAnnotation,
} from '@/lib/kernel-markers';
    ...
    async (editor: unknown, monaco: unknown) => {
      ...
      (monaco as any).languages.registerHoverProvider(language, {
        ...
        provideHover: (model: any, position: any) => {
          ...
          const marker = markers.find((m) => lineNumber >= m.startLine && lineNumber <= m.endLine);
        ...
        run: () => {
          ...
          if (position) {
            const marker = markers.find(
              (m) => position.lineNumber >= m.startLine && position.lineNumber <= m.endLine
            );
            if (marker) {
              alert(
