...
const getStorageKey = (projectId: string) => `project-study-progress-${projectId}`;
...
export function useProjectProgress(projectId: string, chapterIds: string[]) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        const parsed = JSON.parse(stored) as ProjectProgress;
      ...
      console.error(`Failed to load project progress for ${projectId}:`, error);
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error(`Failed to save project progress for ${projectId}:`, error);
    }
  }, [progress, isHydrated, storageKey, projectId]);
  ...
  const resetProgress = useCallback(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        console.error(`Failed to reset project progress for ${projectId}:`, error);
      }
    }
  }, [chapterIds, storageKey, projectId]);
